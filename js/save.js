...
function autoSave() {
  try {
    ...
    const rank = getCurrentRank ? getCurrentRank() : null;
    data._label = `${data.realm||'练气'}${rank?' 「'+rank.name+'」':''} ${data.age||18}岁 ${data.plane==='spirit'?'灵界':data.plane==='immortal'?'仙界':'人界'}`;
...
function _migrateSaveData(data) {
  ...
  if (!data.combatItems) data.combatItems = {};
  ...
  if (data.planeRank === undefined) data.planeRank = data.plane || 'human';
  if (data.renown === undefined) data.renown = 0;
}
