...
function renderSkills() {
  ...
  if (ms) {
    ...
    html += `<div class="card" style="margin-bottom:10px;">
      ...
      <div style="font-size:10px;color:var(--text-tertiary);margin-bottom:8px;">下一层：${growthText}</div>
      ...
      <div style="display:flex;gap:6px;">
        <button class="skill-btn primary" onclick="trainMainSkill()">修炼(${ms.daysPerLv}天)</button>
...
function trainMainSkill() {
  ...
  GameState.skillProgress.main=(GameState.skillProgress.main||0)+ms.daysPerLv;
  ms.level++; GameState.skillProgress.main=0;
  advanceTime(ms.daysPerLv);
  addLog(`修炼${ms.name}至第${ms.level}层`,'success'); renderAll(); renderSkills();
}
...
function doForgetMain(){
  ...
  ms.level=1; GameState.cultivation=Math.floor(GameState.cultivation*0.8); GameState.lifespanMax-=5;
...
function trainSubSkill(id){
  ...
  addLog(`修炼${skill.name}至第${ss.level}层${affinity?' (灵根共鸣！'+(doubleEff?'-50%天数)':'-30%天数)'):''}`,'success'); renderAll(); renderSkills();
}
function learnSubSkill(id){
  ...
  const bonusLv = GameState.spiritRoot?.bonusLv || 0;
  const extraSlots = Math.floor(getSubSkillEffectVal('extraSlot'));
  const maxSlots=Math.max(2,Math.floor(GameState.wis/5))+bonusSlots+extraSlots;
