...
function getEffectiveVit() {
  ...
  return Math.max(1, GameState.vit + aging.vit);
}

function getEffectiveWis() {
  const aging = getAgingDebuff();
  return Math.max(1, GameState.wis + aging.wis);
}

// ---- 寿命安全写入 ----

function setLifespanMax(s, val) {
  s.lifespanMax = Math.max(50, val);
}
function applyLifespan(s, delta) {
  s.lifespanMax = Math.max(50, s.lifespanMax + delta);
}
  ...
  const body = `
    ...
    <div class="death-grid">
      ...
      <div class="death-card" onclick="closeModal();doRebirthLegacy()">
        ...
        <div class="death-desc">遗留${Math.floor(s.spiritStones*SPIRIT_STONES.source.legacy.ratio)}灵石+1颗丹药于初始地图</div>
...
function doRebirthKeep() {
  ...
  if (items.length === 0 && stashed.length === 0) {
    ...
    GameState.rebirthDebuff += 0.3;
        setLifespanMax(GameState, Math.floor(GameState.lifespanMax * (1 - GameState.rebirthDebuff)));;
    grantStarterMaterials();
...
function doRebirthLegacy() {
  const s = GameState;
  const stones = Math.floor(s.spiritStones * SPIRIT_STONES.source.legacy.ratio);
...
function showPossess() {
  const wisBonus = Math.floor(getEffectiveWis() / 10) * 0.02;
...
function doPossess(tierIdx) {
  ...
  if (roll < rate) {
    ...
    if (realm) {
      setLifespanMax(GameState, realm.lifespan);
...
function resetState(clearPastLives) {
  ...
  s.totalDays = 0; s.age = 18; setLifespanMax(s, 100);
  s.atk = 3; s.def = 3; s.spd = 3; s.wis = 3; s.vit = 3; s.spi = 3;
  s.spiritStones = SPIRIT_STONES.starting; s.plane = 'human';
...
async function _startCultivate(days) {
  ...
  if (msPenalty > 0) {
    applyLifespan(GameState, -Math.floor(days * msPenalty * 10));
  ...
  if (!aborted) {
    ...
    if (stoneGain > 0) {
      const earned = Math.floor(days * stoneGain * SPIRIT_STONES.source.cultivation.stoneGainPerLevel);
  ...
  // 修炼获取声望
  const cultRenown = gainCultivationRenown(aborted ? (days - (GameState.totalDays % days)) : days);
  if (cultRenown > 0) addLog(`修炼中声望 +${cultRenown}`, '');
...
function checkDemon() {
  ...
  if (s.delayDays >= 120) {
    ...
    if (s.lifespanMax < 100 && Math.random() < 0.30) {
      triggerDeath('心魔入体');
      return;
    }
    s.delayDays = 0;
...
function doBreakthrough() {
  ...
  if (s.subLevel < realm.subLevels) {
    ...
    const renResult = gainBreakthroughRenown(false);
    if (renResult.gained > 0) addLog(`声望 +${renResult.gained}`, '');
  } else {
    // ---- major breakthrough ----
    // check break item
    if (realm.breakItem) {
    ...
    const wisBonus = Math.floor(getEffectiveWis() / 10) * 0.01;
    ...
    if (roll < successRate) {
      ...
      if (nextRealm) {
        ...
        setLifespanMax(s, nextRealm.lifespan + Math.floor(getSubSkillEffectVal('lifespan') * 100) + Math.floor(getSubSkillEffectVal('lifespanBig')));
        s.delayDays = 0;
        ...
        const renMajor = gainBreakthroughRenown(true);
        if (renMajor.gained > 0) addLog(`声望 +${renMajor.gained}`, '');
        if (renMajor.rankedUp) addLog(`声望升阶！晋升为「${renMajor.newRank.name}」`, 'crit');
      } else {
      ...
      applyLifespan(s, -Math.floor(fp.lifespan * (1 - tribReduce)));
      if (s.age >= s.lifespanMax) {
        triggerDeath('突破反噬');
        return;
      }
      advanceTime(180);
...
function _doAscend() {
  ...
  if (nextRealm) {
    ...
    setLifespanMax(GameState, nextRealm.lifespan);
  }
  ...
  // 飞升声望 + 重置至新位面阶级
  const ascRenown = addRenown('ascend');
  if (ascRenown.gained > 0) addLog(`飞升扬名，声望 +${ascRenown.gained}`, 'loot');
...
function checkDeath() {
  // sudden death check (elderly death risk, progressive)
  const pct = GameState.lifespanMax > 0 ? GameState.age / GameState.lifespanMax : 0;
  let deathChance = 0;
  if (pct >= 0.98) deathChance = 0.03;
  else if (pct >= 0.95) deathChance = 0.01;
  else if (pct >= 0.90) deathChance = 0.005;
  else if (pct >= 0.85) deathChance = 0.003;
