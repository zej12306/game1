...
function selectRegion(regionId) {
  ...
  openModal('长途跋涉', `前往${name}，途经${stops.slice(0,-1).join('→')}，共${totalDays}天。确定吗？`, [
    { label: '取消', onclick: 'closeModal()' },
    { label: '出发', onclick: `closeModal();chainTravel('${regionId}',${JSON.stringify(path)})`, primary: true },
  ]);
}
...
function chainTravel(targetId, path) {
  ...
  if (Math.random() < 0.06) {
    ...
    if (ev < 0.3) { const loss=Math.min(s.spiritStones,30); s.spiritStones-=loss; events.push(`途中遭遇劫修！灵石-${loss}`); }
    else if (ev < 0.6) { s.spiritStones+=50; events.push('途中发现灵药，灵石+50'); }
...
function exploreNode(regionId, nodeId) {
  ...
  let fightChance = node.events.fight || 0;
...
function travelTo(regionId) {
  ...
  if (Math.random() < 0.03) {
    const ev = Math.random();
    if (ev < 0.3) { const loss = Math.min(s.spiritStones,20); s.spiritStones-=loss; addLog(`途中遭遇劫修！灵石-${loss}`,'danger'); }
    else if (ev < 0.6) { s.spiritStones+=30; addLog('途中发现灵药，灵石+30','loot'); }
